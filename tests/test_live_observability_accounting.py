...
def test_dry_run_trade_lifecycle_is_separated_from_live_trades_csv(tmp_path):
    ...
    assert dry_rows[0]["resolution_method"] == "binance_fallback"
...
def test_live_market_price_loss_path_does_not_charge_entry_cost_twice(monkeypatch, tmp_path):
    ...
    # Live buy already deducted the entry cost from bankroll: 24.84 - 5.53.
    ...
    assert polybot.stats.total_pnl == -5.53
    assert polybot.stats.bankroll == 19.31
    assert polybot.tracker.resolve_calls[0]["resolution_method"] == "market_price"
